...
def compute_threading(func, x, result_list):
...
class TCM_attn(nn.Module):
    def __init__(self, conv_dim, trans_dim, type='SW'):
        """ SwinTransformer and Conv Block
        """
        super(TCM_attn, self).__init__()
        self.conv_dim = conv_dim
        self.trans_dim = trans_dim
        self.head_dim = 4
        self.window_size = 4
        self.type = type
        assert self.type in ['W', 'SW']
        self.trans_block = Block(self.trans_dim, self.trans_dim, self.head_dim, self.window_size, 0, self.type)
        self.conv1_1 = nn.Conv2d(self.conv_dim+self.trans_dim, self.conv_dim+self.trans_dim, 1, 1, 0, bias=True)
        self.conv1_2 = nn.Conv2d(self.conv_dim+self.trans_dim, self.conv_dim+self.trans_dim, 1, 1, 0, bias=True)
        nn.init.constant_(self.conv1_2.weight,0)
...
class TCM(CompressionModel):
    ...
    def forward(self, x):
        ...
        for slice_index, y_slice in enumerate(y_slices):
            ...
            #if self.training:
            #    y_hat_slice = UniverseQuant.apply(y_slice-mu) + mu
            #else:
        ...
        return {
            ...
            "para":{"means": means, "scales":scales, "y_hat":y_hat}
        }

    def forward_latent_e2e(self, x, latent_e2e_net):
        y = self.g_a(x)
        y_shape = y.shape[2:]
        y = latent_e2e_net.g_a(y)
        ...
        scales = torch.cat(scale_list, dim=1)
        y_likelihoods = torch.cat(y_likelihood, dim=1)
        y_hat = latent_e2e_net.g_s(y_hat)
        x_hat = self.g_s(y_hat)
    ...
    def forward_fromlatent(self, y, z):
        y_shape = y.shape[2:]
        _, z_likelihoods = self.entropy_bottleneck(z)
    ...
    def test_fromlatent(self, y, z):
        ...
        return {"strings": [y_strings, z_strings], "shape": z.size()[-2:], "x_hat": x_hat}
    ...
    def compress(self, x):
        ...
        enc_nettime = time.time()-t0
        encoder.encode_with_indexes(symbols_list, indexes_list, cdf, cdf_lengths, offsets)
        y_string = encoder.flush()
        y_strings.append(y_string)

        return {"strings": [y_strings, z_strings], "shape": z.size()[-2:], "enc_nettime": enc_nettime}
    ...
    def decompress(self, strings, shape):
        ...
        return {"x_hat": x_hat,"dec_nettime": dec_nettime}



def subpel_conv3x3_tiny(in_ch: int, r: int = 1) -> nn.Sequential:
    """3x3 sub-pixel convolution for up-sampling."""
    return nn.Sequential(
     nn.PixelShuffle(r),
     nn.Conv2d(in_ch// r ** 2, in_ch, kernel_size=3, padding=1),
...
class hs(nn.Module):
    def __init__(self, num_slices, enc=True):
        super(hs, self).__init__()
        self.num_slices = int(num_slices)

        # self.hs_atten = nn.ModuleList(
        #     nn.Sequential(
        #         #conv(32 + (32 // self.num_slices) * min(i, 5), 24, stride=1, kernel_size=3)
        #         SWAtten((320 + (320//self.num_slices)*min(i, 5)), (32 + (32//self.num_slices)*min(i, 5)), 8, 4,0, inter_dim=128)
        #     ) for i in range(self.num_slices)
        #     )

        self.hs_transforms = nn.ModuleList(
            nn.Sequential(
                conv(320 + (320//self.num_slices)*min(i, 5), 24, stride=1, kernel_size=3),
                nn.GELU(),
                conv(24, 12, stride=1, kernel_size=3),
                nn.GELU(),
                conv(12, (320//self.num_slices), stride=1, kernel_size=3),
            ) for i in range(self.num_slices)
        )
        for i in range(self.num_slices):
            nn.init.constant_(self.hs_transforms[i][-1].weight, 0)
            nn.init.uniform_(self.hs_transforms[i][-1].bias, -3, -2.3)

    def forward(self, x, slice_idx):
        w = self.hs_transforms[slice_idx](x)
        #self.hs_atten[slice_idx](x)
        return w


class fcn(nn.Module):
    def __init__(self,layers,lc):
        super(fcn, self).__init__()

        self.n = int(layers)
        self.latent_channels = int(lc)
        self.fc_layers = nn.ModuleList([])

        for i in range(self.n):
            if i==0:
                self.fc_layers.append(nn.Linear(1,self.latent_channels))
            else:
                self.fc_layers.append(nn.Linear(self.latent_channels,self.latent_channels))

        nn.init.constant_(self.fc_layers[-1].weight,0)
        nn.init.constant_(self.fc_layers[-1].bias, 0)
...
class mask(nn.Module):
    def __init__(self,layers,lc):
        ...
        self.lc = int(lc)
        self.lmd_map = fcn(layers,lc)
        self.latent_attn = ConvTransBlock(lc//2, lc//2, 32, 4, 0, 'SW')
        self.alpha = nn.Parameter(torch.randn(1))
        nn.init.constant_(self.alpha,0)

    def forward(self, y, lmd):
        b = y.size()[0]
        lmd_embedding = self.lmd_map(lmd)
        lmd_embedding = torch.reshape(lmd_embedding,(b, self.lc, 1, 1))
        rd_info = self.latent_attn(y)
        return lmd_embedding + self.alpha*rd_info
...
class LAE(nn.Module):
    def __init__(self,layers, in_channels):
        ...
        self.n = int(layers)
        self.mask = mask(self.n, self.m)

    def forward(self, feature, lmd):
        mask = self.mask(feature, lmd)
...
class TCM_VBR(CompressionModel):
    def __init__(self, config=[2, 2, 2, 2, 2, 2], head_dim=[8, 16, 32, 32, 16, 8], drop_path_rate=0, N=128, M=320,
        ...
        self.modnet_ga = LAE(2, M)
        self.modnet_gs = LAE(2, M)
    ...
    def forward(self, x, lmd_test=-1):
        b = x.size()[0]
        if lmd_test >= 0:
            lmd_index = torch.ones((b,1)).cuda().to(torch.float32)
            lmd_index *= lmd_test

        else:
            lmd_index = np.random.randint(6, size=[b], dtype=np.int)
            lmd_index = torch.tensor(lmd_index).cuda().to(torch.float32).reshape([b, 1, 1, 1])

        y = self.g_a(x)
        y = self.modnet_ga(y, lmd_index)
        ...
        y_hat = self.modnet_gs(y_hat, lmd_index)
        x_hat = self.g_s(y_hat)
    ...
    def forward_fromlatent(self, y, z, lmd_index):
    ...
    def test_fromlatent(self, y, z, lmd_index):
        enc_time_s = time.time()
        self.update()
        y_shape = y.shape[2:]
        ...
        enc_time = time.time() - enc_time_s
        dec_time_s = time.time()
        ...
        y_hat = self.modnet_gs(y_hat, lmd_index)
        x_hat = self.g_s(y_hat).clamp_(0, 1)
        dec_time = time.time() - dec_time_s

        return {"strings": [y_strings, z_strings], "shape": z.size()[-2:], "x_hat": x_hat, "time": [enc_time, dec_time]}
    ...
    def compress(self, x, lmd_test):
        b = x.size(0)
        lmd_index = torch.ones((b, 1)).cuda()
        lmd_index *= lmd_test

        y = self.g_a(x)
        y_shape = y.shape[2:]
        y, m_y = self.modnet_ga(y, lmd_index)
        ...
        return {"strings": [y_strings, z_strings], "shape": z.size()[-2:], "m_y": torch.mean(torch.log(m_y))}
    ...
    def decompress(self, strings, shape, lmd_test):
        lmd_index = torch.ones((1, 1)).cuda()
        lmd_index *= lmd_test
        z_hat = self.entropy_bottleneck.decompress(strings[1], shape)
        ...
        y_hat = torch.cat(y_hat_slices, dim=1)
        y_hat,_ = self.modnet_gs(y_hat, lmd_index)
        x_hat = self.g_s(y_hat)
